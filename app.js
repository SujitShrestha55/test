...
function renderInventory() {
  ...
  ${headHtml("INVENTORY", "Products", "Manage your supermarket inventory.",
    `<div style="display:flex;gap:10px;">
      <button class="secondaryBtn" id="invScanBtn">${icon("camera", 16)} Scan to Add/Restock</button>
...
function attachContentListeners() {
  ...
  if (state.page === "Inventory") {
    ...
    if (addBtn) addBtn.addEventListener("click", () => {
      const code = prompt("Scan or type the product's barcode:");
      if (code === null) return; // cancelled
      routeInventoryBarcode(code);
    });

    const scanBtn = $("#invScanBtn");
    if (scanBtn) scanBtn.addEventListener("click", () => openCameraScanner("inventory"));
  }
}

// Barcode-aware inventory add/restock.
// - Barcode already exists  -> just ask how many units to add to stock (no name/price/etc).
// - Barcode is new          -> ask for the product's details, then create it.
function routeInventoryBarcode(code) {
  const barcode = (code || "").trim();
  if (!barcode) { alert("No barcode entered."); return; }

  const existing = state.products.find(p => p.barcode === barcode);

  if (existing) {
    const addQty = Number(prompt(`"${existing.name}" already exists (current stock: ${existing.stock}).\nHow many units are you adding?`, "10"));
    if (!addQty || addQty <= 0) return;
    existing.stock += addQty;
    renderPage();
    alert(`Stock updated: ${existing.name} now has ${existing.stock} units.`);
    return;
  }

  // New barcode -> needs full details.
  const name = prompt(`New barcode (${barcode}). Enter the product name:`);
  if (!name) return;
  const category = prompt("Category", "General") || "General";
  const price = Number(prompt("Selling price", "100")) || 0;
  const stock = Number(prompt("Initial stock", "10")) || 0;

  state.products.push({ id: Date.now(), barcode, name, category, price, stock });
  renderPage();
...
let scanMode = "pos"; // "pos" -> auto-add to cart, "inventory" -> add/restock flow
let scanLocked = false; // guards against the continuous decode loop firing more than once per scan
...
function openCameraScanner(mode = "pos") {
  ...
  const heading = $("#scanModalTitle");
  if (heading) heading.textContent = mode === "inventory" ? "Scan barcode to add/restock" : "Scan barcode with camera";
...
async function startCameraScan() {
  ...
  try {
    ...
    scanControls = await zxingReader.decodeFromConstraints(constraints, videoEl, (result, err) => {
      // BUG FIX: decodeFromConstraints keeps calling this callback on every video
      // frame (many times a second) for as long as the same barcode stays in view.
      // Without a lock, that fired scanBarcode()/routeInventoryBarcode() repeatedly
      // for a single scan (e.g. adding the item to the cart several times). We now
      // stop the reader and lock out further results the instant we get one match.
      if (result && !scanLocked) {
        ...
        if (scanMode === "inventory") {
          closeCameraScanner();
          routeInventoryBarcode(text);
        } else {
          const matched = state.products.some(p => p.barcode === text);
          if (matched) {
            scanBarcode(text); // auto-adds the matched item straight to the cart
            setTimeout(closeCameraScanner, 350);
          } else {
            statusEl.className = "scanStatus err";
            statusEl.textContent = `No product found for barcode ${text}. Try again or add it in Inventory.`;
            $("#scanRetryBtn").classList.add("show");
          }
        }
      }
